		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				var html= '';
					...
					html += '					<a style="height:'+para.itemHeight+';width:90px;" title="点击添加文件" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
					html += '						<div class="uploadImg" style="width:'+imgWidth+'px">';
					html += '							<i class="iconfont icon-shangchuantupian" style="font-size: 50px; color: #FE5400"></i>';
					html += '						</div>';
					html += '					</a>';
					html += '				</div>';
					html += '			</div>';
	            ...
	            $(self).append(html).css({"width":para.width,"height":"90px"});
...
//				}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "../../Application/Mobile/Public/uploadapi/control/images/fileType/";
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		</div>';
					html += '	</div>';
					html += '	<a style="height:'+para.itemHeight+';width:90px;" href="#" class="imgBox">';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						filelist +=file.index+":"+response+";"
						//alert("file.index=="+file.index);
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#file.indexuploadList_" + file.index).fadeOut();
							// 重新设置统计栏信息
						...
						k++;
						if(k==filecount) {
							if(filelist.length>0) filelist=filelist.substring(0,filelist.length-1);
							form1.filelist.value=filelist;
							form1.submit();
						}
					},
